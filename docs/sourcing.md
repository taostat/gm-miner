...
So for `zai/glm-5.2` at a buyer retail of $1.40 in / $4.40 out, declaring any of
its routes at `--discount-pct 5` pays you:
...
Your margin is that figure minus what the upstream charges you. `gmcli sources`
shows the buyer retail in the `BUYER RETAIL / MTOK` column so you can compute the
left-hand side; the right-hand side is between you and the upstream, and it is
what differs between routes.

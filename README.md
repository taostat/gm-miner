...
|---|---|---|
| `image/` | W4 | Envoy-only miner image with three provider routes (Anthropic / OpenAI / Gemini) and an optional `benchmark` route to a synthetic upstream. Pinned to digest. The runtime is `envoy` plus its native `/stats/prometheus` exposure — no sidecar service. |

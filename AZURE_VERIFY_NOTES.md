...
The owner-capture checks bind the Azure account to the configured endpoint by ARM identity, require every catalog-named deployment to serve its exact canonical model in the adapter's format, and reject secondary storage or monitoring sinks (`userOwnedStorage`, `raiMonitorConfig`). For Azure OpenAI, they also require the ARM-reported RAI policy mode to be `Asynchronous_filter` or `Deferred` for each Azure-governed deployment. This verifies configuration; it does not prove that inference responses are never buffered server-side. These checks run at container startup; the full sweep repeats every 15 minutes and deployment binding is polled every minute. A policy violation detected after startup terminates `attestd` and restarts the container.

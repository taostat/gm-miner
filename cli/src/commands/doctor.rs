...
fn provider_keys_check(cfg: &Config) -> Check {
    let Some(keys) = cfg.provider_keys.as_ref() else {
        return Check::fail(
            ...
            "no usable provider keys — run `gmcli set-api-keys --anthropic <key>` (and/or --openai / --google / --chutes / --zai / --moonshot / --deepinfra / --kubetee / --engy, or configure Bedrock/Azure upstreams)",
        );
    };
    if !keys.any_set() {
        return Check::fail(

...
| `set-api-keys` | Persist provider API keys (Anthropic, OpenAI, Google, Chutes, Z.ai, Moonshot, DeepInfra, KubeTEE) and cloud-upstream selectors (Bedrock, Microsoft Foundry, Azure OpenAI) to `~/.gmcli/config.json` |
...
- A sourcing route is one of possibly several upstreams that serve a buyer product (e.g. both `deepinfra/zai-org/GLM-5.2` and `engy/glm-5.2` serve `zai/glm-5.2`); none is canonical, and the buyer product id names the model, not the host. The router draws one row per worker per buyer pool with the direct route winning, so a single worker can serve only ONE route per buyer product — two upstreams for one model need two workers. Source products are filtered out of the buyer-facing `GET /products`, so `sources` and `declare-product` read them from `GET /miners/products/sources` instead. Settlement is on the *buyer* product's retail less the declared discount, which is why the declaration output labels it `Retail (buyer)`. `declare-products` (the fan-out) stays catalog-only on purpose: a source offer commits the miner to an upstream it holds a key for, so it must be declared one at a time. A registry deployed before that endpoint answers 404 (it returns 401 unauthenticated and an empty 200 otherwise), which the CLI reads as "no routes" — during a rollout `declare-product` must still reject a mistyped model as a mistyped model.
- **Adding a supplier (a new upstream provider) touches two repos, and `docs/sourcing.md` is the step that gets forgotten.** In this repo: the `Provider` enum (`cli/src/types.rs`), the key field and accessor (`cli/src/config.rs`), the slot fan-out (`cli/src/slots.rs`), the `set-api-keys` flag (`cli/src/main.rs`), the envoy route + cluster + SNI (`image/envoy.yaml`), the key guard and `fan_out_slots` call (`image/start.sh`), the compose env passthrough (`dstack/docker-compose.yaml`), `CANONICAL_ALLOWED_ENVS` and its length in `cli/src/compose_hash.rs` (which changes the attested `compose_hash`, so re-anchor `REGISTRY_TESTNET_COMPOSE_HASH` and the `start_render_test.rs` render golden in the same PR), and — if the supplier serves an existing buyer product rather than its own — a row in the `docs/sourcing.md` matrix plus the provider list in `README.md` §3 and the `set-api-keys` row above. In the `gm` repo: `SOURCE_ROUTES` (`registry/src/gm_registry/source_aliases.py`), the curated price tables (keyed by `(provider, model)` — a bare model id is not unique across upstreams), the `product.json` provider enum (then codegen), and the control-loop probe tables. `gm`'s CI fails if a `SOURCE_ROUTES` pair is missing from this repo's `docs/sourcing.md`, so the docs step is enforced — but only for sourcing routes, not for a supplier that gets its own buyer product.
